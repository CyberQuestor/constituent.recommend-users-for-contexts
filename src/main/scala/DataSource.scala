  ...
  def getItems(sc: SparkContext): RDD[(String, Item)] = {
    ...
    val itemsRDD: RDD[(String, Item)] = PEventStore.aggregateProperties(
      ...
      val item = try {
        ...
        Item(id= entityId, categories = properties.getOpt[List[String]]("categories"), properties.getOrElse[String]("domain",""), properties.getOrElse[String]("type",""), properties.getOrElse[String]("vehicleType",""))
      } catch {
        case e: Exception => {
          logger.error(s"Failed to get properties ${properties} of" +
